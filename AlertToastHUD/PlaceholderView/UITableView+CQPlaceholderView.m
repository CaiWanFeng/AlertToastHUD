//
//  UITableView+CQPlaceholderView.m
//  AlertToastHUD
//
...
@implementation UITableView (CQPlaceholderView)
...
- (void)cq_reloadData {
    ...
    // 如果声明了numberOfSectionsInTableView，section为0时说明是空列列表
    if ([dataSource respondsToSelector:@selector(numberOfSectionsInTableView:)]) {
        NSInteger section = [dataSource numberOfSectionsInTableView:self];
        if (section == 0) {
            // 组数为0，一定是空列表
            isEmptyList = YES;
        } else if (section == 1) {
            // 只有一组，看cell数量
            NSInteger cellCount = [dataSource tableView:self numberOfRowsInSection:1];
        ...
        // 没有声明numberOfSectionsInTableView
        // 默认section为1
